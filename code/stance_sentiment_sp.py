...
def create_resample(train_sequence,train_enc,train_enc_senti,train_embeddings):
    df = pd.DataFrame(list(zip(train_sequence,train_enc,train_enc_senti,train_embeddings)), columns =['text','pol', 'sent','use'],index=None)
    blv=(df[df['pol'] == 0])
...
for train_index,test_index in kf.split(total_sequence,total_labels_pol_enc):
    ...
    model=Model([input1_pol,input2_pol,input1_senti,input2_senti],[act_specific_output,senti_specific_output])
    ...
    model.fit([train_sequence,train_embeddings,train_sequence,train_embeddings],[train_pol,train_senti], shuffle=True,validation_split=0.2,epochs=10,verbose=2)

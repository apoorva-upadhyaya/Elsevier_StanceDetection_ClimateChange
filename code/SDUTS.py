...
for i in range(len(train_data)):
    ...
    train_temp.append((train_data.temp.values[i]))
...
for i in range(len(test_data)):
    ...
    test_temp.append((test_data.temp.values[i]))
...
label_encoder=LabelEncoder()
final_lbls=train_temp+test_temp
values=array(final_lbls)
total_integer_encoded=label_encoder.fit_transform(values)

train_enc_temp=total_integer_encoded[0:len(train_text)]
test_enc_temp=total_integer_encoded[len(train_text):]
total_integer_encoded=to_categorical(total_integer_encoded)

train_temp=total_integer_encoded[0:len(train_text)]
print("train_temp::::",train_temp[:5])
test_temp=total_integer_encoded[len(train_text):]
...
total_labels_temp= np.vstack((train_temp,test_temp))
...
list_acc_pol,list_acc_senti,list_acc_temp=[],[],[]
list_f1_pol,list_f1_senti,list_f1_temp=[],[],[]
list_prec_pol=[]
list_rec_pol=[]
list_prec_senti=[]
list_rec_senti=[]
...
for train_index,test_index in kf.split(total_sequence,total_labels_pol_enc):
    ...
    test_temp,train_temp=total_labels_temp[test_index],total_labels_temp[train_index]
    ...
    input_text_senti = Embedding(vocab_size, 768, weights=[embedding_matrix], input_length=29, name='text_share_embed_senti')(input1_senti)
    ...
    input2_senti = Input (shape = (512, ))
    input_senti = Reshape((512,1))(input2_senti)
    lstm2_senti = Bidirectional(LSTM(100, name='lstm_inp2_senti', activation='tanh',dropout=.2,kernel_regularizer=regularizers.l2(0.07)))(input_senti)
    text_use_senti = Dense(100, activation="relu")(lstm2_senti)
    ...
    model=Model([input1_pol,input2_pol,input1_senti,input2_senti,input1_temp,input2_temp],[act_specific_output,senti_specific_output,temp_specific_output])


    ##Compile
    model.compile(optimizer=Adam(0.0001),loss={'task_specific_act':'binary_crossentropy','task_specific_senti':'categorical_crossentropy','task_specific_temp':'categorical_crossentropy'},
    loss_weights={'task_specific_act':1.0,'task_specific_senti':0.5,'task_specific_temp':0.3}, metrics=['accuracy'])    
    print(model.summary())

    model_architecture = model.to_json()
    #save the model architecture to JSON file
    with open('stance_all.json', 'w') as json_file:
        json_file.write(model_architecture)


    model.fit([train_sequence,train_embeddings,train_sequence,train_embeddings,train_sequence,train_embeddings],[train_pol,train_senti,train_temp], shuffle=True,validation_split=0.2,epochs=12,verbose=2)
    predicted = model.predict([test_sequence,test_embeddings,test_sequence,test_embeddings,test_sequence,test_embeddings])
    print(predicted)
    ...
    sent_pred_specific=predicted[2]
    result_=sent_pred_specific
    p_1 = np.argmax(result_, axis=1)
